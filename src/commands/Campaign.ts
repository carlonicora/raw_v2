...
export const Campaign: Command = {
  ...
  run: async (
    ...
    const campaignTitle: string | undefined = interaction.options.get("title")
      ?.value as string;
    if (!campaignTitle) throw new Error("Missing campaign name");

    try {
      const campaign: campaigns | null = await database.getCampaign(
        interaction,
      );
      ...
      content
        ...
        .setDescription(
          `A campaign (${campaign.name}) already exists on this server.`,
        );
    } catch (error: any) {
