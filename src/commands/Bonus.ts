      ...
      options: [
        {
          name: "player",
          description:
            "Select the player to whose character the bonus will be assigned",
...
const sessionBonus = async (
  ...
  if (interaction.user.id !== campaign.discordUserId)
    throw new Error(
      "You don't have permission to provide bonus. Only the storyteller can.",
    );
...
const assignBonus = async (
  ...
  const character: characters = await database.getCharacter(
    interaction,
    user.id,
  );
  ...
  const content = new EmbedBuilder()
    ...
    .setDescription(
      `${user}, your character ${character.name} received a 3 points bonus.`,
    );
...
const rollBonus = async (
  ...
  if (character.bonus <= 0)
    throw new Error("You do not have bonus points to roll your ability.");
  ...
  } their ${abilityName} ability${
    successes > 0 ? " by " + successes.toString() + " points" : ""
  }.`;

  if (successes > 0) description += `It is now at ${abilityValue} points.`;

  const content = new EmbedBuilder()
    .setColor(successes > 0 ? Colour.Green : Colour.Red)
    .setTitle(
      `${
        successes > 0 ? "Successfully increased " : "Failed to increase "
      } ${abilityName}`,
    )
...
const upBonus = async (
  ...
  if (character.bonus <= 0)
    throw new Error("You do not have bonus points to up your ability.");
  ...
  if (!characterAbility)
    throw new Error(
      "You can up only an ability your character already have. Roll for it first.",
    );
